...
export type Experience = { slug: string; period: string; company: string; role: string; summary: string; outcomes: string[]; stack: string[]; logo?: string; sidebarMedia?: SidebarMedia[]; evidence?: string[]; related?: string[]; links?: DetailLink[]; };
export type Project = { slug: string; period: string; track: string; title: string; role: string; hook: string; summary: string; outcomes: string[]; stack: string[]; visual?: SidebarMedia; sidebarMedia?: SidebarMedia[]; evidence?: string[]; related?: string[]; links?: DetailLink[]; linkLabel?: string; href?: string; };
...
export const dict = {
  en: {
    ...
    section: {
      ...
      philosophyQuote: "기계가 인간의 맥락을 100% 이해할 때, 기술은 비로소 인간을 대체하는 것이 아니라 해방시키는 가장 강력한 레버리지가 됩니다.",
      education: 'Education',
    ...
    philosophyData: [
      { 
        id: 'STRUCTURE', images: ['/media/r2c-chart.png'],
  ...
  ko: {
    ...
    philosophyData: [
      { 
        id: 'STRUCTURE',
        title: '인간 지식의 지능화', images: ['/media/r2c-chart.png'], 
    ...
    projectData: [
      ...
      { slug: 'insight-flow', period: '2026', track: 'ai product systems', title: 'InsightFlow (인사이트 플로우)', role: '프로덕트 포지셔닝 및 워크플로우 설계', hook: 'AI는 단순한 기능 추가가 아니라 워크플로우 전체를 소유할 때 진정한 가치를 갖습니다.', summary: '프로젝트 생성, 설문 설계, 응답자 흐름, 전사 분석을 단일 AI 네이티브 시스템으로 통합한 한국어 정성조사 프로덕트입니다.', outcomes: ['프로젝트 생성부터 분석까지 단일 워크플로우 구현', 'Supabase 기반 풀스택 프로덕트 아키텍처 설계'], stack: ['Next.js', 'TypeScript', 'Supabase'], href: 'https://github.com/Jun0zo/insight-flow' },
      { slug: 'snappo', period: '2025 - 2026', track: 'consumer product', title: 'Snappo (스내포)', role: '프로덕트 컨셉 및 회수 흐름 설계', hook: '어떤 프로덕트는 저장 공간에서 시작하지만, 이 프로덕트는 기억, 소유권, 그리고 회수(Recall)에서 시작했습니다.', summary: '6자리 코드를 통해 사진을 회수하는 사진 기억 플랫폼을 탐구했습니다. 감성적 브랜딩과 가벼운 소비자 UX가 교차하는 프로덕트입니다.', outcomes: ['갤러리 소유권 대신 코드 기반 회수 로직 설계', '회수(Recall)를 중심으로 한 소비자 브랜드 방향성 수립'], stack: ['React', 'Vite', 'Tailwind', 'Supabase'], href: 'https://github.com/Astrobear-Lab/Snappo', visual: { src: '/media/snappo-memory-1.jpeg', alt: 'Snappo memory flow' }, sidebarMedia: [{ src: '/media/snappo-memory-1.jpeg', alt: 'Snappo memory flow', caption: 'Consumer memory flow' }, { src: '/media/snappo-memory-2.jpeg', alt: 'Snappo memory asset', caption: 'Photo-memory product texture' }, { src: '/media/snappo-brainstorming.png', alt: 'Snappo brainstorming board', caption: 'Early product ideation' }], evidence: ['Astrobear-Lab repository', 'JunOS node: Project Experience - Snappo', 'Consumer-facing brand and retrieval workflow'], related: ['Project Experience - openfolio', 'Project Experience - yukgap', 'Project Experience - memora'] },
      ...
      { slug: 'openfolio', period: '2025', track: 'open source', title: 'openfolio', role: '메인테이너', hook: '포트폴리오는 정적인 문서가 아니라 계속해서 살아 움직이는 시스템입니다.', summary: '개발자들이 자신의 작업물을 템플릿화하고 자산화할 수 있도록 돕는 오픈소스 포트폴리오 관리 시스템을 개발했습니다.', outcomes: ['재사용 가능한 오픈소스 포트폴리오 템플릿 구축'], stack: ['Next.js', 'React', 'Tailwind'] },
      { slug: 'gps-ros-parcel-drone', period: '2019 - 2020', track: 'robotics autonomy', title: 'GPS ROS Parcel Drone', role: '제어 구현', hook: '초기 로보틱스 경험이 알려준 것은 단순했습니다. GPS만으로는 현실을 충분히 설명할 수 없습니다.', summary: 'GPS/ROS 기반 택배 드론에서 marker 기반 정렬, dropzone 로직, 물리 제어 제약을 연결했습니다. 이후 JunOS evidence audit에서 X-Optimus 산불 감시 드론과 분리된 별도 프로젝트로 정리했습니다.', outcomes: ['교육혁신 Festival 국회의원상 수상', 'GPS / marker / dropzone 코드가 로컬 evidence로 보존됨'], stack: ['ROS', 'GPS', 'ArUco', 'Drone Control'], evidence: ['JunOS split-fixed node: GPS ROS Parcel Drone', 'Local evidence: Projects/competitions/drone_xoptimus', 'Award evidence: Educational Innovation Festival National Assembly Award'], related: ['Project Experience - GPS ROS Parcel Drone', 'Awards and Honors - AWD-2020-EDU-NA'] },
      { slug: 'xoptimus-wildfire-drone', period: '2020 - 2021', track: 'robotics hardware', title: 'X-Optimus Wildfire Drone', role: '하드웨어-제어 빌더', hook: '로봇 지능이 연구 주제가 되기 전, 하드웨어는 현실의 제약을 가장 먼저 가르쳐준 대상이었습니다.', summary: '납땜부터 Arduino 부착까지 직접 구성한 from-scratch 산불 감시 드론 프로젝트입니다. GPS/ROS 택배 드론과는 다른 프로젝트로, X-Optimus 금상과 연결됩니다.', outcomes: ['X-Optimus 융합 교육 프로그램 성과 발표회 금상 수상', 'JunOS evidence audit 이후 GPS/ROS 택배 드론과 분리 정리'], stack: ['Arduino', 'Drone Hardware', 'Soldering', 'Control'], evidence: ['JunOS split-fixed node: X-Optimus Wildfire Drone', 'Award evidence: X-Optimus Gold Prize', 'Program document evidence in Records'], related: ['Project Experience - X-Optimus Wildfire Drone', 'Awards and Honors - AWD-2021-XOPTIMUS-GOLD'] },
      { slug: 'promptaes', period: '2024', track: 'research ai', title: 'PromptAES', role: '연구원', hook: 'LLM이 파인튜닝 없이도 에세이를 공정하게 채점할 수 있을까요?', summary: '교육 콘텐츠를 효율적으로 평가하기 위한 프롬프트 기반 자동 에세이 채점 모델을 연구했습니다.', outcomes: ['자동 에세이 채점 파이프라인 구축'], stack: ['LLM', 'Prompt Engineering', 'Python'], visual: { src: '/media/promptaes-tsne.png', alt: 'PromptAES t-SNE visualization' }, sidebarMedia: [{ src: '/media/promptaes-tsne.png', alt: 'PromptAES t-SNE visualization', caption: 'Embedding structure view' }, { src: '/media/promptaes-prompt1.png', alt: 'PromptAES prompt score plot', caption: 'Prompt-level score distribution' }], evidence: ['JunOS node: Project Experience - PromptAES', 'Prompt-aware AES follow-up experiments', 'R2C / writing evaluation research line과 연결'], related: ['Project Experience - 2stepED', 'Project Experience - promptaes_web', 'Publication - NAACL Findings - R2C'] }
    ],
    publicationData: [
      { slug: 'aes-multilayer-encoders', title: 'Automated Essay Scoring based on Multi-layer Encoders', authors: 'Jo, J., et al.', venue: "EMNLP", date: '2026', details: 'EMNLP 2026 심사 중. 학생 에세이의 계층적 창의성과 일관성을 포착하기 위해 다층 인코더 아키텍처를 활용한 새로운 자동 채점 프레임워크를 제안합니다.', logo: '' },
      { slug: 'r2c-naacl-2025', title: 'Representation-to-Creativity (R2C): Automated Holistic Scoring Model for Essay Creativity', authors: 'Kim, D., Jo, J., On, B.-W., & Lee, I.', venue: 'Findings of ACL: NAACL', date: '2025년 4월', logo: '/media/naacl_logo.png', href: 'https://aclanthology.org/2025.findings-naacl.292/', sidebarMedia: [{ src: '/media/r2c-chart.png', alt: 'R2C Chart' }, { src: '/media/naacl-poster.png', alt: 'NAACL Poster' }, { src: '/media/2steped-distribution.png', alt: 'Score Distribution' }] },
      { slug: 'purdue-drone-privacy-2024', title: 'Evaluating Privacy Infringement Level in Drone-Captured Images Using Privacy Image Quality Assessment Algorithms', authors: 'Jo, J., et al.', venue: 'Purdue University Research', date: '2024년 6월', details: '퍼듀 대학교 방문연구 당시, Privacy-aware AI 프로젝트의 팀장으로서 비전 데이터 기반 사생활 침해 지표 정량화 모델링을 주도했습니다. 아이디어 제안부터 실험 설계, 논문 작성까지 수행하며 연구 리더십을 확립했습니다.', logo: '/media/purdue_logo.png', sidebarMedia: [{ src: '/media/purdue-1.png', alt: 'Purdue Publication 1' }, { src: '/media/purdue-2.png', alt: 'Purdue Publication 2' }, { src: '/media/purdue-3.png', alt: 'Purdue Publication 3' }] },
      { slug: 'offroad-farm-robot-2023', title: '농업 환경에서의 자율주행 로봇 개발을 위한 오프로드 주행환경이 고려된 심층 강화학습 프레임워크', authors: 'Jo, J., et al.', venue: 'KIIT Conference', logo: '/media/kiit_logo.png', date: '2023년 11월', details: '비정형 오프로드 환경에서의 농업용 수확 로봇 자율주행을 위해 Unity 기반의 강화학습 시뮬레이션을 설계하고, 논문경진대회 우수논문상 (금상)' },
      ...
      { slug: 'shortform-subtitle-jkiit-2021', title: '숏폼 콘텐츠 자막 자동 추출 시스템', authors: 'Jo, J., Gim, J., On, B.-W., & Jeong, D.', venue: 'Journal of KIIT', logo: '/media/kiit_logo.png', date: '2021년 6월', href: 'https://doi.org/10.14801/jkiit.2021.19.6.29' }
    ],
    trackSummary: [
      ...
      { title: '프로덕트 시스템 (Product Systems)', description: '데이터의 추론 과정을 노출하고, 워크플로우를 명확히 하며, 복잡한 시스템을 직관적으로 만드는 인터페이스를 구축합니다.', items: ['InsightFlow', 'Snappo', 'openfolio', 'TechGraph'] }
    ]
  }
};
